 ...
 * arrive.  Paths are child objects, which are accessed by the GstChildProxy interface.
 *
 ...
 *
 * Note the way that the pulsesink is set to async=false, so that in the case where data is
 * dropped, it won't prevent the pipeline from prerolling.
 * </refsect2>
 */
...
static void gst_switch_bin_set_sinkpad_drop (GstSwitchBin * switch_bin,
    gboolean do_drop);
static GstPadProbeReturn gst_switch_bin_dropping_pad_probe (GstPad * pad,
    GstPadProbeInfo * info, gpointer user_data);
  ...
  if (switch_bin->num_paths == new_num_paths) {
    ...
    for (i = switch_bin->num_paths; i < new_num_paths; ++i) {
      ...
      switch_bin->paths[i] = path =
          g_object_new (gst_switch_bin_path_get_type (), "name", name, NULL);
      path->bin = switch_bin;
      ...
      GST_DEBUG_OBJECT (switch_bin, "added path #%u \"%s\" (%p)", i, name,
          (gpointer) path);
    }
  } else {
    ...
    for (i = new_num_paths; i < switch_bin->num_paths; ++i) {
      ...
      if (path == switch_bin->current_path) {
        ...
        GST_DEBUG_OBJECT (switch_bin,
            ...
            i, name, (gpointer) (switch_bin->paths[i]));
      }

      gst_child_proxy_child_removed (GST_CHILD_PROXY (switch_bin),
          G_OBJECT (path), GST_OBJECT_NAME (path));
      gst_object_unparent (GST_OBJECT (switch_bin->paths[i]));

      GST_DEBUG_OBJECT (switch_bin, "removed path #%u \"%s\" (%p)", i, name,
          (gpointer) (switch_bin->paths[i]));
    }
    ...
    GstSwitchBinPath * switch_bin_path)
{
  ...
  if (switch_bin_path != NULL) {
    if (switch_bin_path->element != NULL) {
      ...
      GST_DEBUG_OBJECT (switch_bin,
          "pushing stream-start downstream prior to enabling drop probe");
...
static void
gst_switch_bin_set_sinkpad_drop (GstSwitchBin * switch_bin, gboolean do_drop)
{
  GstPad *pad;

  if ((do_drop && (switch_bin->drop_probe_id != 0)) || (!do_drop
          && (switch_bin->drop_probe_id == 0)))
    return;
  ...
  if (do_drop) {
    switch_bin->drop_probe_id =
        gst_pad_add_probe (pad, GST_PAD_PROBE_TYPE_DATA_DOWNSTREAM,
        gst_switch_bin_dropping_pad_probe, switch_bin, NULL);
  } else {
...
static GstPadProbeReturn
gst_switch_bin_dropping_pad_probe (G_GNUC_UNUSED GstPad * pad,
    GstPadProbeInfo * info, G_GNUC_UNUSED gpointer user_data)
{
  ...
  if (info->type & GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM) {
    ...
    switch (GST_EVENT_TYPE (event)) {
      ...
      default:
        GST_DEBUG_OBJECT (switch_bin, "letting through %s event",
